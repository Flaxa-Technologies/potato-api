...
/// Main plugin structure for Linux. Must implement `Default`.
#[derive(Default)]
pub struct MyLinuxPlugin;

impl Plugin for MyLinuxPlugin {
    /// Declare plugin identity, version, author, and description.
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata::new("MyLinuxPlugin", "1.0.0")
            .author("Flaxa Technologies")
            .description("Official Paper-Grade PotatoMC Native Plugin for Linux (.so)")
    }

    /// Called when the shared library (.so) is dynamically loaded by PotatoMC.
    fn on_load(&self, context: &PluginContext) -> Result<(), String> {
        context.logger().info("MyLinuxPlugin loaded on Linux host!");
